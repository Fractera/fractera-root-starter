...
const FILE_SEEDS = ["development-loop-2026.jpg"];
...
// Была ли связь установлена ИМЕННО СЕЙЧАС. Сбрасывать кэш каталога имеет смысл
// только после настоящей записи: при повторном запуске («уже связан») сбрасывать
// нечего, а лишний сброс заставил бы приложение перерисовать всё зря.
let linked = false;
...
try {
  ...
  for (const row of products) {
    ...
    if (patch.ok) linked = true;
  }
} catch (e) {
  console.log(`  привязка товаров пропущена: ${e.message}`);
}

// 🔒 СВЯЗАТЬ В БАЗЕ МАЛО — НАДО СКАЗАТЬ ОБ ЭТОМ ПРИЛОЖЕНИЮ (владелец 2026-08-14:
// «в медиатеке картинки есть, а во вкладке продукты их нет»).
//
// Каталог читается через `unstable_cache` со сроком в ЧАС (`lib/catalogue.ts`).
// Строки, прочитанные до связывания, лежат в этом кэше и не знают о картинках;
// истечение ISR-окна страницы делу не помогает — пересобранная разметка берёт
// те же кэшированные строки и снова рисует прочерки.
//
// На свежем сервере это правило, а не случайность: сборка идёт ДО запуска слоя
// данных, поэтому посев из `prebuild` ничего не находит, каталог собирается с
// прочерками, а связывание случается позже — при старте (`prestart`). Без этого
// вызова первый посетитель видит товары без картинок целый час.
//
// Сброс — best-effort и НИКОГДА не валит посев: приложение в этот момент может
// быть ещё не поднято (мы как раз в его `prestart`), и это нормально — тогда
// картинки покажет ближайшая регенерация.
if (linked) {
  const appUrl = process.env.APP_ORIGIN ?? "http://127.0.0.1:3000";
  const headers = { "Content-Type": "application/json", "x-agent-identity": "seed-media" };
  if (process.env.REVALIDATE_SECRET) headers.Authorization = `Bearer ${process.env.REVALIDATE_SECRET}`;
  try {
    const r = await fetch(`${appUrl}/api/revalidate`, { method: "POST", headers, body: "{}" });
    console.log(`  кэш каталога сброшен: ${r.ok ? "ok" : "ОТКАЗ " + r.status}`);
  } catch (e) {
    console.log(`  кэш каталога сбросить не удалось (не беда): ${e.message}`);
  }
}
