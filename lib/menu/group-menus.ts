...
//
// 🔒 ТОЛЬКО НАДЗАГОЛОВОК ВЕРХНЕГО УРОВНЯ — ДВА ПРОБЕЛА ОТСТУПА, ЯКОРЬ НА НАЧАЛО
// СТРОКИ (2026-09-20).
//
// ✗ ОПЛАЧЕНО ДЕФЕКТОМ, КОТОРЫЙ УВИДЕЛ ВЛАДЕЛЕЦ, А НЕ ПРИБОР. Здесь стояло
// `/eyebrow:\s*'([^']*)'/` без якоря — то есть ПЕРВОЕ совпадение в файле. У
// страницы своего надзаголовка не было, зато он был у секции в середине текста,
// и в меню появилось слово «Блокчейн»: подпись настоящая, файл настоящий, просто
// не та. Отказ молчаливый по природе — пункт меню подписан, ссылка работает, и
// единственный способ заметить подмену — прочитать меню глазами.
//
// 🛑 ВТОРАЯ ПОЛОВИНА ТОГО ЖЕ ДЕФЕКТА: когда совпадений нет вовсе, ниже берётся имя
// папки. Оно НЕ ПЕРЕВОДИТСЯ — так в русском меню оказалась «Architecture». Запасной
// вариант оставлен намеренно (пункт без подписи хуже подписи на чужом языке), но
// теперь это видно как долг, а не как норма: у страницы просто нет своего слова.
function groupLabel(groupDir: string, slug: string, lang: string): string {
  for (const l of [lang, "en"]) {
    const m = read(join(groupDir, "_data", `${l}.ts`)).match(/^ {2}eyebrow:\s*'([^']*)'/m);
    if (m?.[1]) return m[1];
  }
